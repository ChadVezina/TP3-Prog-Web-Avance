<!-- Page pour éditer un forfait existant -->
<script setup>
...
const loading = ref(false); // Indicateur de chargement de la soumission
const loadingData = ref(true); // Indicateur de chargement des données
...
// Chargement des données du forfait au montage du composant
onMounted(async () => {
  ...
  try {
    // Récupération des données du forfait depuis l'API
...
// Fonction pour modifier le forfait existant
...
<template>
  <div class="min-h-screen bg-linear-to-b from-purple-50 to-white">
    <!-- En-tête de la page -->
    <div
      class="bg-linear-to-r from-purple-600 to-purple-800 text-white py-12 px-4 shadow-lg"
    >
      <div class="max-w-4xl mx-auto">
        <div class="flex items-center gap-4 mb-3 animate-fade-in">
          ...
          <div>
            <h1 class="text-4xl font-bold">Modifier le forfait</h1>
            <p class="text-purple-100 mt-2">
              Mettez à jour les informations de votre forfait touristique
            </p>
          </div>
        </div>
      </div>
    ...
    <div class="max-w-4xl mx-auto px-4 py-8">
      ...
      <div
        v-if="loadingData"
        class="bg-white rounded-xl shadow-md p-12 text-center animate-fade-in"
      >
        <div
          class="inline-block animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-purple-600 mb-4"
        ></div>
        <p class="text-gray-600 text-lg">Chargement du forfait...</p>
      </div>
          ...
          <div class="lg:col-span-1">
            <div class="bg-white rounded-xl shadow-md p-6 sticky top-8">
              ...
              <div class="border-2 border-gray-200 rounded-lg overflow-hidden">
                <!-- Image preview -->
                <div
                  class="relative h-40 bg-linear-to-br from-purple-400 to-purple-600 overflow-hidden"
                >
                  <img
                  ...
                  <div v-if="form.categorie" class="absolute top-2 right-2">
                    <span
                      class="inline-block bg-white/90 backdrop-blur-sm text-purple-800 text-xs font-semibold px-2 py-1 rounded-full shadow"
                    >
                      {{ form.categorie }}
                    </span>
                  </div>
